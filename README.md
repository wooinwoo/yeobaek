...
flowchart LR
    ...
    subgraph Next["Next.js 16 App Router"]
        ...
        API["Route Handler<br/>/api/posts<br/>/api/posts/:id/comfort"]
        Zod["Zod 스키마<br/>(입력 검증)"]
    end

    Prisma["Prisma 7 Client<br/>+ libSQL Adapter"]
    DB[("libSQL<br/>로컬 file: / Turso")]

    Browser -- "최초 요청 (SSR)" --> RSC
    RSC -- "findMany" --> Prisma
    Browser -- "글 작성 / 위로 (fetch)" --> API
    API --> Zod
    API -- "create / update" --> Prisma
    Prisma --> DB
    DB --> Prisma
    Prisma --> RSC
    Prisma --> API
...
2. **SSR + 클라이언트 낙관적 업데이트**
   첫 페인트는 서버 컴포넌트의 SSR로 빠르게 그리고, 변경은 명시적 REST 엔드포인트로 분리했습니다. 위로 클릭은 낙관적으로 즉시 반영하고 실패 시 롤백해 체감 반응성과 정합성을 함께 챙겼습니다.

3. **Zod 런타임 입력 검증**
   `lib/posts.ts`에 작성 스키마(`createPostSchema`)를 정의하고 API에서 `safeParse`로 검증합니다. 실패 시 사람이 읽을 수 있는 메시지와 함께 `422`를, JSON 파싱 실패엔 `400`을 반환합니다. 스키마에서 `z.infer`로 입력 타입을 도출해 검증 규칙과 타입이 한 곳에서 일치합니다.

4. **Tailwind v4 `@theme` 디자인 토큰**
...
pnpm db:seed                 # 예시 글 시드 (이미 글이 있으면 건너뜀)
...
│           └── [id]/comfort/    # POST 위로 +1
├── components/
│   ├── Header.tsx · Footer.tsx
│   ├── CommunityBoard.tsx       # 작성·필터·낙관적 업데이트
│   └── Reveal.tsx               # 스크롤 등장 애니메이션
├── lib/
│   ├── db.ts                    # Prisma + libSQL 싱글톤
│   ├── posts.ts                 # Zod 스키마 · PostDTO 타입
